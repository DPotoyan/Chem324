...
::: {.fragment}
Quantum mechanics is a by-product of the search for a better light bulb. [The full story, with the people and instruments (video)](https://www.youtube.com/watch?v=i1TVZIBj7UA)
:::
:::
::::
...
- Energy at frequency $\nu$ = **number of waves** of that frequency $\times$ **average energy** of each
- **Factor 1** is geometry: how many standing waves fit in a box. Same classically and quantum
- **Factor 2** is thermodynamics: this is where classical and quantum physics part ways

## Factor 1: counting the waves in a box

![](../../ch01/images/mode_counting.png){width="80%" fig-align="center"}

- Shorter waves fit more easily: $N \sim L/\lambda \sim \nu$ in 1D; fit along each direction, so $\sim \nu^3$ in 3D
- A thin slice of frequency holds $\; dN_{\nu} = \dfrac{8\pi}{c^3}\,\nu^2\, d\nu$

## Factor 2, the classical way: equipartition
...
- Modes $\times$ energy per mode, the **Rayleigh-Jeans law**:
$$\rho({\nu}) = \frac{8\pi}{c^3}\nu^2 \cdot k_B T$$
:::
::::
...
:::
::: {.column width="45%"}
- $\nu^2 \times k_BT$ **shoots to infinity** at high $\nu$
- Total radiation would be **infinite**: a light bulb could destroy the universe
- Factor 1 is solid geometry, so **factor 2 must be wrong**
:::
::::
...
## Planck's law: same recipe, new factor 2
...
::: {.fragment}
Modes $\times$ energy per mode, with factor 1 untouched:
$$\rho_{\nu}(T) = \frac{8\pi \nu^2}{c^3} \cdot \frac{h\nu}{e^{\frac{h\nu}{kT}} - 1}$$
