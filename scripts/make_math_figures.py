...
px, py = 3.0, 2.0; pr, pth = np.hypot(px, py), np.arctan2(py, px)
fig, (c1, c2) = plt.subplots(1, 2, figsize=(10.5, 4.2))
for g in range(-1, 5):
    c1.axvline(g, color="#e3e5e8", lw=0.9); c1.axhline(g, color="#e3e5e8", lw=0.9)
c1.plot([0, px], [py, py], "--", color=GRAY, lw=1.2); c1.plot([px, px], [0, py], "--", color=GRAY, lw=1.2)
c1.plot([0, px], [0, 0], color=CARDINAL, lw=3.4, solid_capstyle="butt")
c1.plot([px, px], [0, py], color=TEAL, lw=3.4, solid_capstyle="butt")
c1.plot(px, py, "o", color=BLACK, ms=9)
c1.text(px + 0.15, py + 0.12, "P", fontsize=15, fontweight="bold")
c1.text(px / 2, -0.42, "x = 3", color=CARDINAL, fontsize=14, ha="center", fontweight="bold")
c1.text(px + 0.15, py / 2 - 0.1, "y = 2", color=TEAL, fontsize=14, fontweight="bold")
c1.set_title("Rectangular: how far across, how far up", fontsize=13, fontweight="bold", color=BLACK)

tt = np.linspace(0, 2 * np.pi, 300)
for rr in [1, 2, 3, 4]:
    c2.plot(rr * np.cos(tt), rr * np.sin(tt), color="#e3e5e8", lw=0.9)
for a in np.arange(0, 2 * np.pi, np.pi / 6):
    c2.plot([0, 4.2 * np.cos(a)], [0, 4.2 * np.sin(a)], color="#e3e5e8", lw=0.9)
arc2 = np.linspace(0, pth, 60); c2.plot(1.15 * np.cos(arc2), 1.15 * np.sin(arc2), color=CARDINAL, lw=2.4)
c2.annotate("", xy=(px, py), xytext=(0, 0), arrowprops=dict(arrowstyle="->", color=TEAL, lw=3.2))
c2.plot(px, py, "o", color=BLACK, ms=9)
c2.text(px + 0.15, py + 0.12, "P", fontsize=15, fontweight="bold")
c2.text(0.95, 1.02, "r = 3.6", color=TEAL, fontsize=14, fontweight="bold", rotation=np.degrees(pth), rotation_mode="anchor")
c2.text(1.35, 0.28, r"$\theta = 34^\circ$", color=CARDINAL, fontsize=14, fontweight="bold")
c2.set_title("Polar: how far out, at what angle", fontsize=13, fontweight="bold", color=BLACK)
for cc in (c1, c2):
    cc.axhline(0, color=BLACK, lw=1.0); cc.axvline(0, color=BLACK, lw=1.0)
    cc.set_xlim(-1.4, 4.4); cc.set_ylim(-1.4, 4.0); cc.set_aspect("equal"); cc.set_xticks([]); cc.set_yticks([])
    cc.spines["left"].set_visible(False); cc.spines["bottom"].set_visible(False)
fig.tight_layout(rect=(0, 0.09, 1, 1))
fig.text(0.5, 0.035, r"trigonometry is the dictionary:   $x = r\cos\theta$,   $y = r\sin\theta$",
         fontsize=15, ha="center", color=BLACK)
fig.savefig(f"{OUT}/two_coordinates.png", dpi=200)

# ---------------------------------------------------------------- fundamental theorem of algebra
xs = np.linspace(-2.1, 2.1, 400)
fig, (g1, g2) = plt.subplots(1, 2, figsize=(10.5, 4.2))
for shift, col, lab in [(-1, CARDINAL, r"$x^2 - 1$"), (0, GRAY, r"$x^2$"), (1, TEAL, r"$x^2 + 1$")]:
    g1.plot(xs, xs**2 + shift, color=col, lw=2.8, label=lab)
g1.axhline(0, color=BLACK, lw=1.6)
g1.plot([-1, 1], [0, 0], "o", color=CARDINAL, ms=10, zorder=5)
g1.plot([0], [0], "o", color=GRAY, ms=10, zorder=5)
g1.annotate("", xy=(0, 1), xytext=(0, 0), arrowprops=dict(arrowstyle="<->", color=TEAL, lw=1.6))
g1.text(0.10, 0.40, "gap", color=TEAL, fontsize=12)
g1.text(1.75, 0.12, "y = 0", fontsize=12, color=BLACK)
g1.set_xlim(-2.4, 2.4); g1.set_ylim(-1.5, 4.3); g1.set_xlabel("x")
g1.set_xticks([-2, -1, 0, 1, 2]); g1.set_yticks([])
g1.spines["left"].set_visible(False)
g1.legend(fontsize=12, loc="upper center", ncol=3, frameon=False, handlelength=1.4, columnspacing=1.2)
g1.set_title("On the real line: 2, 1, or 0 crossings", fontsize=13, fontweight="bold", color=BLACK)

g2.axhline(0, color=BLACK, lw=1.0); g2.axvline(0, color=BLACK, lw=1.0)
tt2 = np.linspace(0, 2 * np.pi, 200); g2.plot(np.cos(tt2), np.sin(tt2), "--", color="#d7dade", lw=1.2)
g2.plot([-1, 1], [0, 0], "o", color=CARDINAL, ms=11, zorder=5)
g2.plot([0], [0], "o", color=GRAY, ms=11, zorder=5)
g2.plot([0, 0], [-1, 1], "o", color=TEAL, ms=11, zorder=5)
g2.text(1.10, 0.14, r"$+1$", color=CARDINAL, fontsize=14); g2.text(-1.40, 0.14, r"$-1$", color=CARDINAL, fontsize=14)
g2.text(0.13, 1.06, r"$+i$", color=TEAL, fontsize=14); g2.text(0.13, -1.26, r"$-i$", color=TEAL, fontsize=14)
g2.text(0.14, -0.30, "0 twice", color=GRAY, fontsize=12)
g2.set_xlim(-1.9, 1.9); g2.set_ylim(-1.75, 1.75); g2.set_aspect("equal")
g2.set_xlabel("Real"); g2.set_ylabel("Imaginary"); g2.set_xticks([-1, 0, 1]); g2.set_yticks([-1, 0, 1])
g2.set_title("In the plane: always exactly 2 roots", fontsize=13, fontweight="bold", color=BLACK)
fig.tight_layout(); fig.savefig(f"{OUT}/fta_roots.png", dpi=200)
print("done (two coordinate systems, fundamental theorem of algebra)")
